...
const OnlinePaymentOptions = ({ courseData, basePrice, whatsappMessage }) => {
	...
	const handleOrder = async () => {
		if (!isAuthenticated) {
			localStorage.setItem("onlineCourseClick", "true")
			...
			if (selectedOption === "") {
				...
				if (selectedOption === "prepaid") {
					try {
						localStorage.setItem('coursePurchaseOrder', 'online')
					...
					window.location.href = "/fgiit/online-check-out"
				}
			}
		}
	}
	...
	useEffect(() => {
		const itemCart =
			JSON.parse(localStorage.getItem("onlineCourseClick")) === true
